...
| 7 | success_rate 95% 이상 | act/evaluate 단계의 성공률. 95% 미만이면 결과 신뢰도 부족 → 재실행 |
...
- success_rate = (성공 대화 수 / 전체 시도 대화 수) × 100
- 95% 이상: 정상 → Phase 3 진행
- 90~94%: 경고 → 결과는 사용하되 로그 확인 필요
- 90% 미만: 재실행 필수 → API 상태, rate limit, 네트워크 확인
- summary.json의 execution_stats.success_rate 필드에서 확인
- retry 후에도 실패한 건은 failures 배열에 원인과 함께 기록됨
...
| 5 | success_rate 안정적 | 매 실행마다 95% 이상 유지 |
...
| success_rate < 90% | API rate limit 초과 또는 서버 장애 | MAX_CONCURRENT 낮추거나 대기 후 재시도 |
