...
- **React 19 + TypeScript**
- **Tailwind CSS v4** (`@tailwindcss/vite` 플러그인, CSS에 `@import "tailwindcss"` 만 사용)
- **Recharts** (차트)
- Vite 빌드
- **HeroUI 사용 금지** — plain Tailwind CSS 컴포넌트만 사용

### 디자인 원칙

- **항상 밝은 테마 (Light Theme)** — 다크 모드 사용 금지
  - 페이지 배경: `bg-gray-50` 또는 `bg-slate-50`
  - 카드: `bg-white` + `shadow-sm` + `border border-gray-200`
  - 텍스트: `text-gray-900` (제목), `text-gray-600` (보조)
  - 헤더: `bg-white` + `border-b border-gray-200`
  - 액센트: cyan/indigo(주), green(성공), red(위험)
...
H. Strategy Recommendation Report — 실전 세일즈 전략 추천 리포트
   (전략별 실전 가이드, 고객유형별 매트릭스, Do's & Don'ts, Markdown 다운로드)
I. Evolution Panel (🧬 전략 진화 트래커) — 세대별 자가개선 과정 시각화
   (점수 진화 추이 LineChart, 클러스터별 Δ 변화량, 세대별 전략 상세 + 학습,
    레이더 차트 vs 베이스라인 비교, keep/discard 상태)
```
...
```
0a. ui-ux-pro-max 디자인 시스템 생성 (필수 — 코드 작성 전)
    python3 ~/.claude/skills/ui-ux-pro-max/scripts/search.py \
      "analytics dashboard data visualization persuasion simulation" \
      --design-system -p "Ralphthon Dashboard" --persist
    python3 ~/.claude/skills/ui-ux-pro-max/scripts/search.py "heatmap line-chart" --domain chart
    python3 ~/.claude/skills/ui-ux-pro-max/scripts/search.py "animation accessibility" --domain ux
    → design-system/MASTER.md 생성
    → 이후 모든 UI 코드는 이 디자인 시스템을 따른다

0b. heroui-react 컴포넌트 문서 사전 확인
    node ~/.claude/skills/heroui-react/scripts/list_components.mjs
    node ~/.claude/skills/heroui-react/scripts/get_component_docs.mjs Card Table Button Accordion Chip Badge Select
    node ~/.claude/skills/heroui-react/scripts/get_theme.mjs
    → HeroUI v3 컴포넌트 API 파악
    → MASTER.md 색상 팔레트를 HeroUI oklch 테마 변수에 매핑

1. 프로젝트 초기화 (Vite + React + TS + HeroUI v3 + Tailwind v4 + Recharts)
2. 데이터 로딩 레이어 (runs/ JSON 파싱)
3. Layout + Header (HeroUI Card + TextField + Select + Button)
4. Strategy Leaderboard (A) — HeroUI Table + Chip + Badge
5. Heatmap (B) — CSS Grid 직접 구현 (히트맵만 커스텀, 나머지는 HeroUI)
6. Cell Explanation (C) + Cluster Insight (D) — HeroUI Card + Accordion
7. Persona Drilldown (E) + Experiment Trend (F) — HeroUI Card + Recharts
8. Conversation Viewer (G) — HeroUI Card + ScrollShadow
9. Strategy Recommendation Report (H) — 실전 세일즈 전략 추천 리포트 + MD 다운로드
10. ui-ux-pro-max Pre-Delivery 체크리스트 검증
11. 빌드 + 동작 확인
12. 코드 리뷰 + 동작 검증 (Codex 서브에이전트)
13. 리뷰 피드백 반영 + 최종 빌드
14. 오케스트레이터 시각 검증 (Hermes가 직접 브라우저로 확인)
```
...
- [ ] ui-ux-pro-max 디자인 시스템 생성 및 적용 (`design-system/MASTER.md` 존재)
...
- [ ] Strategy Recommendation Report(H)가 실전 세일즈 가이드를 표시
- [ ] Markdown 리포트 다운로드 기능 동작
...
└── output/               ← 에이전트가 생성하는 코드. 하네스 바깥.
    ...
    ├── scripts/
    ├── config/
    ├── frontend/          ← Phase 4에서 생성. React + HeroUI + Tailwind 대시보드.
