...
def _default_strategy(iteration: int) -> dict:
    return {
        'strategy_id': f'iteration-{iteration}-strategy',
        'hypothesis': '생활 밀착형 가치 제안',
        'tone': '공감적이고 구체적',
        'funnel': {
...
async def hypothesize_strategy(product: dict, previous_learnings: list[str] | None = None, iteration: int = 1) -> dict:
    ...
    base = _default_strategy(iteration)
    base.update(parsed)
    base.setdefault('funnel', _default_strategy(iteration)['funnel'])
    return base
